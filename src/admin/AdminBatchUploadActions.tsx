...
export default function AdminBatchUploadActions({
  ...
  const [buttonText, setButtonText] = useState('Добавить все фото');
    ...
    try {
      ...
      for await (const data of readStreamableValue(stream)) {
        setButtonText(addedUploadCount.current === 0
          ? `Добавление 1 из ${storageUrls.length}`
          : `Добавление ${addedUploadCount.current + 1} из ${storageUrls.length}`,
        );
        ...
        <div className="w-full space-y-4 py-1">
          <div className="flex">
            <div className="grow text-main">
              {showBulkSettings
                ? `Применить теги к ${pluralize(storageUrls.length, 'фото')}`
                : `Найдено ${pluralize(storageUrls.length, 'фото')}`}
            </div>
            <FieldSetWithStatus
          ...
          <div className="space-y-2">
            <ProgressButton
              ...
              onClick={async () => {
                ...
                if (confirm(`Вы уверены что хотите добавить ${storageUrls.length} фото?`)) {
                  setIsAdding(true);
